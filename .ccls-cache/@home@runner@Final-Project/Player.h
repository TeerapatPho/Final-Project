...
public:
    ...
    bool get_and_change_fullness(bool = false);
...
bool player::get_and_change_fullness(bool fullness_input)
{
...
int player::add_item(string obj_name){
    return bag[obj_name] == true;
}
bool player::buying(int cost)
{
    if (bag["VexPhone"]) {
        ...
        if (money > cost) {
            ...
            cout << "You need " << cost-money << " dollar more!" << endl;
            return false;
        }
...
void player::gen_card(vector<card> card_temp)
{
     for(int i=0;i<card_temp.size()*20;i++){
        if(i%20==0) cout<<"    ";
        else cout << "=";
     }
     cout << "\n";
